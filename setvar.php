...
function authReq($exten, $pass, $passedkey)
{
    ...
    if ($debug == 1) {
        fputs($fp, "tengo en sesion el contexto $context\n");
    }
    ...
    $now = date('r');
    if ($debug == 1) {
        fputs($fp, "$now\topening manager connection on host $host, port $port\n");
    }
    $sk  = fsockopen($host, $port, $errnum, $errstr, $timeout);
    $now = date('r');
    if ($debug == 1) {
        fputs($fp, "$now\tconnection to manager ok!\n");
    }
    if ( ! is_resource($sk)) {
        if ($debug == 1) {
            $now = date('r');
            fputs($fp, "$now\texit zero, could not connect!\n");
        }
        ...
        if ($debug == 1) {
            $now = date('r');
            fputs($fp, "$now\tantes de fputs $exten,$pass,$passmd5\n");
        }
        ...
        while ( ! feof($sk)) {
            ...
            if ($debug == 1) {
                $now = date('r');
                fputs($fp, "$now\tduring while, response: $dati\n");
            }
        }
    }
    fclose($sk);
    ...
    if ($debug == 1) {
        $now = date('r');
        fputs($fp, "$now\tclosing manager connection\n");
    }
    ...
    if ($debug == 1) {
        $now = date('r');
        fputs($fp, "$now\tFinal response after trim: ($dati)\n");
    }
    if ($dati == "ok") {
        return 1;
    } else {
...
if (isset($_POST['sesvar'])) {
    ...
    if ( ! in_array($variable, $valid_vars)) {
        if ($debug == 1) {
            $now = date('r');
            fputs($fp, "$now\tsetting $variable dissallowed as it is not in allowed list\n");
            fclose($fp);
        }
        die('no way');
    }
    ...
    if ($variable == "key") {
        if ( ! authReq($_POST['exten'], $_POST['pass'], $_POST['value'])) {
            if ($debug == 1) {
                $now = date('r');
                fputs($fp, "$now\tBad authentication. Exiting...\n");
                fclose($fp);
            }
            die("bad auth");
        }
        ...
        if ($variable != "extension" || $variable != "language") {
            if ( ! isset($_SESSION[MYAP]['key'])) {
                if ($debug == 1) {
                    $now = date('r');
                    fputs($fp, "$now\tsetting $variable dissallowed as no key was set\n");
                    fclose($fp);
                }
                die("no key set");
            }
    ...
    if ($debug == 1) {
        $now = date('r');
        fputs($fp, "$now\tsetting variable $variable = $value\n--------------------------------\n");
        fclose($fp);
    }
    session_write_close();
    die("ok");
